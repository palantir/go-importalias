...
func NewProjectImportInfo() ProjectImportInfo {
	return &projectImportAliasInfo{
		importInfos: make(map[string]map[string]aliasInfo),
	}
}
...
func (p *projectImportAliasInfo) addImportAlias(file, alias, importPath string, pos token.Position) {
	if _, ok := p.importInfos[importPath]; !ok {
		p.importInfos[importPath] = make(map[string]aliasInfo)
	}
...
func (p *projectImportAliasInfo) ImportsToAliases() map[string][]aliasInfo {
	m := make(map[string][]aliasInfo)
...
func (p *projectImportAliasInfo) FilesToImportAliases() map[string][]singleAlias {
	m := make(map[string][]singleAlias)
	for importPath, aliases := range p.importInfos {
